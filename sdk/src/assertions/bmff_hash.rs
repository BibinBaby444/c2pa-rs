...
/// Maximum hash bytes in the single-file fragment Merkle leaf row.
...
impl BmffHash {
    ...
    fn single_file_merkle_layout(
        ...
        let hash_size = hash_alg_size_in_bytes(alg)?;
        if (fragments.len() as u64).saturating_mul(hash_size) > MAX_MERKLE_LEAVES_SIZE {
            return Err(Error::BadParam(
