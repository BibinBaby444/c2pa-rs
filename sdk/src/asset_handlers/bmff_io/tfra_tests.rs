...
fn parse(bytes: &[u8]) -> (Arena<BoxInfo>, HashMap<String, Vec<Token>>) {
    let mut input = Cursor::new(bytes);
    let ftyp = read_ftyp_box(&mut input).unwrap();
    input.rewind().unwrap();
    let size = bytes.len() as u64;
    let (mut tree, root) = Arena::with_data(BoxInfo {
        path: String::new(),
        parent: None,
        offset: 0,
        size,
    ...
    build_bmff_tree(&mut input, size, &mut tree, &root, &mut map, &mut 0, &ftyp).unwrap();
    (tree, map)
}

// Deliberately omit moov: moov+moof uses the separate single-file relocator.
// These are offset-table synthetics, not decodable media or new layout support.
fn synthetic(version: u8, middle: &[u8]) -> Vec<u8> {
    ...
    assert!(!map.contains_key("/moov"));
    assert_eq!(map["/moof"].len(), 3);
    bytes
}
...
fn assert_targets(original: &[u8], current: &[u8]) {
    let (old_tree, old_map) = parse(original);
    let (new_tree, new_map) = parse(current);
    ...
    for (old, new) in old_moofs.iter().zip(new_moofs) {
        let old = &old_tree[*old].data;
        let new = &new_tree[*new].data;
    ...
    for (old, new) in old_map["/mfra/tfra"].iter().zip(&new_map["/mfra/tfra"]) {
        ...
        for _ in 0..count {
            ...
            let index = old_moofs
                .iter()
                .position(|t| old_tree[*t].data.offset == offset)
                .expect("original TFRA entry must point to an actual moof");
            let target = new_tree[new_moofs[index]].data.offset;
    ...
    for version in [0, 1] {
        for later in [false, true] {
            ...
            for size in [64, 256, 4, 4] {
                ...
                io.write_cai(&mut Cursor::new(&current), &mut output, &vec![0; size])
                    .unwrap();
                current = output.into_inner();
                assert_targets(&original, &current);
                assert_eq!(
                    io.read_cai(&mut Cursor::new(&current)).unwrap(),
                    vec![0; size]
                );
            }
            let mut output = Cursor::new(Vec::new());
            io.remove_cai_store_from_stream(&mut Cursor::new(&current), &mut output)
                .unwrap();
...
fn public_xmp_and_placeholder_adjust_tfra() {
    ...
    for version in [0, 1] {
        for later in [false, true] {
            let mut middle = Vec::new();
            if later {
                write_xmp_box(&mut middle, MIN_XMP.as_bytes()).unwrap();
            }
            let original = synthetic(version, &middle);
            let mut current = original.clone();
            for padding in [32, 128, 0, 0] {
                let uri = format!("https://example.test/{}", "x".repeat(padding));
                let mut output = Cursor::new(Vec::new());
                io.embed_reference_to_stream(
                    &mut Cursor::new(&current),
                    &mut output,
                    RemoteRefEmbedType::Xmp(uri.clone()),
                )
                .unwrap();
                current = output.into_inner();
                assert_targets(&original, &current);
                let xmp = io.read_xmp(&mut Cursor::new(&current)).unwrap();
                assert_eq!(
...
#[test]
fn bibin_fragmented_fixture_preserves_intended_moofs() {
    let original = include_bytes!("../../../tests/fixtures/fragmented_mfra.mp4");
    let (tree, map) = parse(original);
    ...
    let info = &tree[map["/mfra/tfra"][0]].data;
