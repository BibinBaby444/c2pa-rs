...
pub(crate) fn single_file_fragment_track_id(input: &mut dyn CAIRead) -> Result<usize> {
    ...
    for moof in map.get("/moof").ok_or_else(unsupported)? {
        let trafs: Vec<_> = map
            ...
            .into_iter()
            .flatten()
            .filter(|t| tree[**t].data.parent == Some(*moof))
            .collect();
        if trafs.len() != 1 {
            return Err(unsupported());
        }
        let tfhds: Vec<_> = map
            ...
            .into_iter()
            .flatten()
            .filter(|t| tree[**t].data.parent == Some(*trafs[0]))
            .collect();
        if tfhds.len() != 1 {
            return Err(unsupported());
        }
        let tfhd = &tree[*tfhds[0]].data;
        ...
        for tfra_token in tfra_list {
            ...
            let trailing_size =
                (length_size_of_traf_num + length_size_of_trun_num + length_size_of_sample_num + 3)
                    as usize;
            let remaining = box_end.saturating_sub(output.stream_position()?);
            if u64::from(num_entries) > remaining / (field_size * 2 + trailing_size as u64) {
                return Err(Error::InvalidAsset(
                    "Bad BMFF: tfra entries exceed box bounds".to_string(),
                ));
            }
...
pub(crate) fn build_bmff_tree<R: Read + Seek + ?Sized>(
    ...
    *recursion_level += 1;
    ...
    while current < end {
        // A single-file fragment may end with a short raw suffix. Keep it out
        // of the box tree but include it in the final fragment's EOF hash.
        if end - current < HEADER_SIZE
            && bmff_tree[*current_node].data.parent.is_none()
            && bmff_path_map.contains_key("/moov")
            && bmff_path_map.contains_key("/moof")
        {
...
impl CAIWriter for BmffIO {
    fn write_cai(
        ...
        let offset_adjust = i64::try_from(new_c2pa_box_size)
            .and_then(|new| i64::try_from(end - start).map(|old| new - old))
            .map_err(|_| Error::InvalidAsset("Bad BMFF offset adjustment".to_string()))?;
    ...
    fn remove_cai_store_from_stream(
        ...
        let offset_adjust = -i64::try_from(end - start)
            .map_err(|_| Error::InvalidAsset("Bad BMFF offset adjustment".to_string()))?;
    ...
    fn embed_reference_to_stream(
        ...
        match embed_ref {
            crate::asset_io::RemoteRefEmbedType::Xmp(manifest_uri) => {
                ...
                // calc offset adjustments
                let offset_adjust = i64::try_from(new_xmp_box_size)
                    .and_then(|new| i64::try_from(end - start).map(|old| new - old))
                    .map_err(|_| Error::InvalidAsset("Bad BMFF offset adjustment".to_string()))?;
...
pub(crate) fn inject_placeholder(
    ...
    // calc offset adjustments
    let offset_adjust = i64::try_from(free_box_bytes.len())
        .map_err(|_| Error::InvalidAsset("Bad BMFF offset adjustment".to_string()))?;
